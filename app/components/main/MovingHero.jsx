...
export default function MovingHero({ reviewCount, avgRating, isCarrier }) {
          ...
          {isCarrier ? (
            <>
              ...
              <Link
                ...
                className="block w-full bg-gray-900 hover:bg-gray-700 text-white font-bold py-4 rounded-xl uppercase tracking-widest text-sm text-center transition-all duration-200"
              >
                Browse Jobs
              </Link>
              <Link
                href="/dashboard/carrier"
                className="block w-full text-center text-sm text-gray-500 hover:text-gray-900 mt-3 underline"
              >
            ...
            <>
              ...
              <p className="text-gray-500 text-sm mt-2 mb-6">
                Compare quotes from verified carriers. Any size, any move —
                fast, safe, reliable.
              </p>
              ...
              <div ref={dropdownRef} className="relative">
                ...
                {open && (
                  <div
                    role="listbox"
                    className="absolute top-full left-0 w-full bg-white border border-gray-200 shadow-2xl z-50 rounded-xl overflow-hidden max-h-90 overflow-y-auto mt-1"
                  >
                    {JOB_TYPES.map((opt) => (
                      <button
                        key={opt.value}
