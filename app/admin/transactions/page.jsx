          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Carrier ID</th>
              ...
              {transactions.map((tx) => (
                <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${TYPE_STYLES[tx.type] ?? 'bg-gray-100 text-gray-500'}`}>
                  ...
                  <td className="px-5 py-4 text-gray-400 text-xs font-mono">
                    {tx.carrier_id ? tx.carrier_id.slice(0, 8) + '...' : '—'}
                  </td>
                  <td className="px-5 py-4 text-right">
                    <span className={`font-bold ${tx.type === 'topup' ? 'text-green-600' : 'text-red-500'}`}>
